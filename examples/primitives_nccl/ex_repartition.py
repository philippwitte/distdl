...
from distdl.backends.nccl.partition import MPIPartition
...
distributed.init_process_group('mpi')
P_world = MPIPartition(group=torch.distributed.new_group(backend='mpi'))
P_world.barrier()

# Create the input partition (using the first 4 workers)
in_shape = torch.tensor([2, 1])
in_size = in_shape.prod()
in_workers = torch.arange(0, in_size)
