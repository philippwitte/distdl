...
class MPIPartition:
    ...
    def __init__(self, group=None, root=None, backend='mpi'):
        ...
        if self._group != None:
            self.active = True
            self.rank = torch.distributed.get_rank(group=self._group)
            self.size = torch.distributed.get_world_size(group=self._group)
        ...
        self.shape = torch.tensor([self.size], dtype=torch.int)
    ...
    def barrier(self):
        r"""Call the MPI Barrier function for that group.
...
#             print_sequential(self._comm, val)
    ...
    def create_partition_inclusive(self, ranks):
        ...
        *in the new group*.
        ...
        group = torch.distributed.new_group(ranks=ranks.tolist(), backend=self.backend)

        return MPIPartition(group, root=self._root, backend=self.backend)
